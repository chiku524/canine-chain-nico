...
func (msg *MsgAddClaimer) ValidateBasic() error {
	...
	if prefix != AddressPrefix {
		return errorsmod.Wrapf(sdkerrors.ErrInvalidAddress, "invalid creator prefix (%s)", fmt.Errorf("%s is not a valid prefix here. Expected `jkl`", prefix))
	}
	...
	if prefix != AddressPrefix {
		return errorsmod.Wrapf(sdkerrors.ErrInvalidAddress, "invalid claimer prefix (%s)", fmt.Errorf("%s is not a valid prefix here. Expected `jkl`", prefix))
	}
