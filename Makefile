...
test-unit:
	@VERSION=$(VERSION) $(GO_CMD) test -short -mod=readonly -modfile=$(MODFILE) -tags='ledger test_ledger_mock test' ./...

test-race:
	@VERSION=$(VERSION) $(GO_CMD) test -mod=readonly -modfile=$(MODFILE) -race -tags='ledger test_ledger_mock test' ./...

test-cover:
	@$(GO_CMD) test -mod=readonly -modfile=$(MODFILE) -timeout 30m -race -coverprofile=coverage.txt -covermode=atomic -tags='ledger test_ledger_mock test' ./...
