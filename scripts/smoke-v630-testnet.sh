...
# Force JSON for AutoCLI/module queries (default output is text/YAML).
q() {
  canined query "$@" --output json
}
...
q upgrade plan 2>/dev/null | jq . || echo "(no scheduled upgrade — OK on fresh net)"

echo "== v630 smoke: applied upgrades =="
for name in v600 v610 v620 v630; do
  if q upgrade applied "$name" 2>/dev/null | grep -q '"name"'; then
    echo "  applied: $name"
  fi
done

echo "== v630 smoke: consensus params =="
q params subspace base consensus 2>/dev/null | jq . || echo "(skipped — use comet RPC for consensus params on SDK 0.54)"
...
if q bank balances "$VALIDATOR_ADDR" 2>/dev/null | jq .; then
  :
elif curl -sf "${REST_API}/cosmos/bank/v1beta1/balances/${VALIDATOR_ADDR}" 2>/dev/null | jq .; then
  :
else
  echo "(failed — enable API in app.toml or set REST_API)" >&2
  exit 1
fi
...
q filetree params | jq .

echo "== v630 smoke: rns params =="
q rns params 2>/dev/null | jq . || true

echo "== v630 smoke: oracle feeds =="
q oracle list-feeds 2>/dev/null | jq . || true

echo "== v630 smoke: wasm code list =="
q wasm list-code 2>/dev/null | jq '.code_infos | length' || true

echo "== v630 smoke: IBC clients =="
q ibc client states 2>/dev/null | jq '.client_states | length' || true

echo "== v630 smoke: jklmint params =="
q jklmint params 2>/dev/null | jq . || true
