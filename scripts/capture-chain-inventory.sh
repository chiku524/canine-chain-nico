...
WASM="$(curl -sf --max-time 60 "${REST_API}/cosmwasm/wasm/v1/code?pagination.limit=500" || echo '{}')"
IBC_CHANNELS="$(curl -sf --max-time 60 "${REST_API}/ibc/core/channel/v1/channels?pagination.limit=200" || echo '{}')"
IBC_CLIENTS="$(curl -sf --max-time 60 "${REST_API}/ibc/core/client/v1/client_states?pagination.limit=100" || echo '{}')"
UPGRADE="$(curl -sf --max-time 30 "${REST_API}/cosmos/upgrade/v1beta1/applied_plan" || echo '{}')"
...
jq -n \
  ...
  --argjson wasm "$WASM" \
  --argjson ibc_channels "$IBC_CHANNELS" \
  --argjson ibc_clients "$IBC_CLIENTS" \
  ...
  '{
    ...
    applied_upgrade: $upgrade,
    wasm_code_count: ($wasm.code_infos // [] | length),
    wasm_codes: [($wasm.code_infos // [])[] | {code_id: .code_id, creator: .creator, data_hash: .data_hash}],
    ibc_channel_count: ($ibc_channels.channels // [] | length),
    ibc_channels: [($ibc_channels.channels // [])[] | {state: .state, ordering: .ordering, counterparty: .counterparty, connection_hops: .connection_hops, port_id: .port_id, channel_id: .channel_id, version: .version}],
    ibc_client_count: ($ibc_clients.client_states // [] | length),
    ibc_clients: [($ibc_clients.client_states // [])[] | {client_id: .client_id, client_state: .client_state["@type"] // .client_state.type_url // "unknown"}]
  }' > "$OUT_FILE"
